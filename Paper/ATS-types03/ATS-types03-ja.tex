...
\noindent {\it Proof.}
By induction by h(D). In particular, Lemma 2 is needed to establish (1).

The type soundess of ATS rests upon the following two theorems, who proofs
are largely standard and thus omitted here.

\noindent {\bf Theorem 1 (Subject Reduction).}
Assume both D :: Σ; P ~ ; ∆ ` S d : s and d ,→ d 0 . Then Σ; P ~ ; ∆ ` S d : s is derivable.

\noindent {\bf Theorem 2 (Progress).}
Assume D :: ∅; ∅; ∅ ` S d : s. Then d is a value, or d ,→ d 0 holds for some dynamic term d 0 , or d = E[dcf (v 1 , . . . , v n )] for some dynamic term dcf (v 1 , . . . , v n ) that is not a redex.
