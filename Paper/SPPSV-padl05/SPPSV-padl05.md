...
* ( _Ṽ_ | _T_ ) → _CT_ は、(関数適用が起きた時の) 現状態が観 _Ṽ_ を満たす場合にのみ、型 _T_ の値に適用できる (動的な) 関数を表わす型です。
